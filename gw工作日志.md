...
- 2026-04-26 22:08:00
  - DOCX 前置信息结构保护继续补强并完成回归
    - 真实问题复核
      - 对比 `特殊幼儿融合教育三维干预路径.docx` 与已下载改写结果，确认旧线上产物确实存在标题拆段、摘要串段、关键词与正文黏连
      - 用当前本地代码对真实源文档逐段分类，已正确识别：
        - 标题
        - `【摘要】` 同段正文
        - `【关键词】` 同段正文
        - `引言`
    ...
    - 测试环境修正
      - `backend/tests/conftest.py`
      - 固定测试态环境变量，避免 pytest 导入阶段被 `.env` 中的 MySQL 配置劫持，改为稳定走 `test/sqlite`
    - 回归补强
      - `backend/tests/test_processing_engine_results.py`
      - 新增“前标签与正文分属不同 run”的 DOCX 回归用例，覆盖真实学术文档常见结构
      - 保留并通过：
        - 标题 / 副标题 / 摘要 / 关键词 / 引言 / 章节标题保护
        - `【摘要】`、`【关键词】` 同段保护
        - split-run 前置信息保护
    - 验证结果
      - 聚焦回归已通过：`3 passed`
...
- 2026-04-27 10:34:00
  - 渠道返佣体系由三级分销收口为二级分销
    - 改造目标
      - 停止新增三级渠道，平台只建一级，一级只可发展二级
      - 团队范围统计不再继续下钻孙级渠道，统一按当前渠道 + 直属下级口径计算
      - 前后端页面文案、层级标签和创建提示同步切换为二级分销口径
      - 保留历史三级渠道的只读展示，避免直接改数据库约束导致线上历史数据写挂
    - 代码调整
      - 后端创建层级上限改为 2：
        - `backend/app/services/partner_rebate_service.py`
      - 渠道团队 / 全部范围统计收口到直属下级：
      ...
      - 渠道树层级标签兼容历史三级展示：
        - `frontend/src/components/partner/PartnerChannelTreeNode.vue`
      - 渠道门户页创建提示、任务引导、层级标签切换为二级分销：
        - `frontend/src/views/partner/PartnerPortalPage.vue`
      - 平台管理后台总览文案、运营提示、层级标签切换为二级分销：
        - `frontend/src/views/admin/AdminPartnerPage.vue`
    - 验证结果
      - 前端已通过 `npm run build`
      - 后端相关文件已通过 `python -m py_compile`
    - 历史数据收口
      - 新增 Alembic 迁移，部署执行后会将历史三级渠道自动提升为对应一级渠道的直属二级
      - 同步修正 `partner_order_attributions.channel_level / root_channel_id / root_channel_code_snapshot`
      - 模型约束与数据库约束统一收紧为 `<= 2`

- 2026-04-27 12:08:00
  - 渠道返佣后台 / 门户可视化与工作台重构
    - 平台后台
      - 重构为“总览数字卡 + 3 个核心图表 + 异常提醒 + 一级渠道经营表”
      - 图表改为：
        - 一级 / 二级新增趋势
        - 一级渠道返佣排行
        - 客户层级归属占比
      - 渠道列表改为经营表格，保留编辑、返佣设置、刷新链接、删除、查看详情
    - 一级渠道门户
      - 重构为“团队经营工作台”
      - 保留新增二级、编辑二级、返佣设置、复制门户/推广文案、提现
      - 新增二级状态标签、催活文案复制、二级贡献图
    - 二级渠道门户
      - 重构为“个人经营工作台”
      - 聚焦客户分发、订单、返佣、提现
      - 去掉团队管理心智，只保留个人经营必需动作
    - 样式与交互
      - 统一系统蓝视觉
      - 手机端改为单列紧凑布局
      - 明细区收进折叠区，默认先给经营结果和关键动作
    - 验证结果
      - 前端已通过 `npm run build`
...
- 2026-04-27 12:21:36
  - 渠道返佣经营数据后端聚合化
    - 后端接口
      - 新增平台经营分析聚合：`/partners/admin/analytics`
      - 新增渠道门户经营分析聚合：`/partners/portal/analytics`
      - 后端直接产出总览卡、趋势序列、返佣排行、客户占比、异常提醒、二级贡献、套餐占比
    - 平台后台
      - 总览数字卡改为直接消费后端 summary
      - 异常提醒改为直接消费后端 anomalies
      - 3 个核心图表改为直接消费后端 growth / rank / mix 数据
      - 新建、编辑、审核提现、打款、删渠道、刷新链接后同步刷新 analytics
    - 渠道门户
      - 经营趋势图改为直接消费后端 trend_series
      - 一级二级贡献图 / 二级套餐占比改为直接消费后端 analytics
      - 行动提示改为基于后端趋势与排行数据生成，不再依赖前端临时拼装
    - 验证结果
...
- 2026-04-27 12:34:58
  - 渠道返佣经营口径修正
    - 平台后台
      - `admin analytics` 对齐渠道列表筛选，`keyword / status` 会同步作用到总览卡、图表、异常提醒
      - 渠道列表筛选改为同时刷新经营分析，不再出现“表格已筛选但上方还是全局数据”
    - 渠道门户
      - `portal analytics` 新增 `scope` 口径，图表和经营提醒改为独立 scope 控制
      - 一级渠道经营提醒不再只看 Top8 二级，改为基于全量直属二级统计待激活数量与返佣产出
      - 二级贡献图仍保留 Top8 展示，但提醒判断已切回全量口径
    - 时间口径
      - 近 14 天趋势分桶改为按北京时间自然日统计，不再直接按 UTC 切天
    - 验证结果
      - 后端已通过 `python -m py_compile backend/app/api/partners.py`
      - 前端已通过 `npm run build`
