...
def _run_preanalysis(db, *, text: str) -> dict:
    payload = _generate_json_payload(
        ...
        prompt=build_cnki_rewrite_precheck_prompt(text),
        error_code=4625,
        error_message="知网降AIGC预分析阶段未返回有效JSON",
    )
...
def _run_prompt_b_validation(db, *, source_text: str, rewritten_text: str) -> dict:
    payload = _generate_json_payload(
        db,
        task_type=TaskType.REWRITE,
        prompt=build_cnki_rewrite_validation_prompt(source_text, rewritten_text),
        error_code=4626,
        error_message="知网降AIGC校验阶段未返回有效JSON",
    )
...
def rewrite(db, *, task: Task | None, text: str, report_summary: dict | None = None) -> dict:
    ...
    try:
        ...
        if not output:
            raise BizError(code=4621, message="知网降AIGC大模型改写失败: Prompt A 输出为空")
        ...
        raise BizError(code=4621, message=f"知网降AIGC大模型改写失败: {exc}") from exc

    if output.strip():
        return {"text": output, "rule_trace": rule_trace}

    raise BizError(code=4622, message="知网降AIGC大模型结果为空")
