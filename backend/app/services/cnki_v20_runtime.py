...
from app.services.cnki_v20_prompt import build_cnki_v20_prompt
...
_STYLE_FLOOR_FORBIDDEN = ("很好", "根子", "拿到", "搞", "弄", "蛮好", "挺好")
_MIN_OUTPUT_LENGTH_RATIO = 0.72
...
@dataclass
class CnkiV20Plan:
    paragraphs: list[CnkiV20ParagraphQuota]
    total_chars: int
    total_quota: int
...
@dataclass
class CnkiV20TextRunResult:
    text: str
    run: CnkiV20RunResult


@dataclass
class CnkiV20TextBlock:
    kind: str
    text: str


@dataclass
class CnkiV20TextLayout:
    blocks: list[CnkiV20TextBlock]
...
def execute_cnki_v20(db, *, task_type: TaskType, paragraphs: list[str]) -> CnkiV20RunResult:
    cleaned_paragraphs = [str(item or "").strip() for item in paragraphs if str(item or "").strip()]
    if not cleaned_paragraphs:
        raise BizError(code=4621, message="知网 V20 无可改写正文")
    plan = build_cnki_v20_plan(cleaned_paragraphs)
    llm_cfg = load_llm_config(db)
    prompt = build_cnki_v20_prompt(
        "\n\n".join(cleaned_paragraphs),
        runtime_context=_build_runtime_context(plan),
    )
    issues: list[str] = []
    raw_output = ""
    for attempt in range(1, 3):
        ...
        try:
            ...
            if attempt >= 2:
                raise
    raise BizError(code=4621, message="知网 V20 执行失败")


def execute_cnki_v20_text(db, *, task_type: TaskType, source_text: str) -> CnkiV20TextRunResult:
    layout = build_cnki_v20_text_layout(source_text)
    if not layout.body_paragraphs:
        raise BizError(code=4621, message="知网 V20 无可改写正文")
    run = execute_cnki_v20(db, task_type=task_type, paragraphs=layout.body_paragraphs)
    rebuilt = layout.rebuild(_split_rewritten_paragraphs(run.text))
    return CnkiV20TextRunResult(text=rebuilt, run=run)


def build_cnki_v20_plan(paragraphs: list[str]) -> CnkiV20Plan:
    normalized = [str(item or "").strip() for item in paragraphs if str(item or "").strip()]
    quotas: list[CnkiV20ParagraphQuota] = []
    total_chars = 0
    for index, paragraph in enumerate(normalized, start=1):
        char_count = count_billable_chars(paragraph)
        total_chars += char_count
        quota = max(3, math.ceil(max(char_count, 1) / 100) * 8)
        quotas.append(CnkiV20ParagraphQuota(index=index, text=paragraph, char_count=char_count, quota=quota))
    total_quota = math.ceil(max(total_chars, 1) / 100) * 8
    return CnkiV20Plan(paragraphs=quotas, total_chars=total_chars, total_quota=total_quota)


def build_cnki_v20_text_layout(source_text: str) -> CnkiV20TextLayout:
    normalized = str(source_text or "").replace("\r\n", "\n").replace("\r", "\n").strip()
    if not normalized:
        return CnkiV20TextLayout(blocks=[])
...
def _build_runtime_context(plan: CnkiV20Plan) -> str:
    return (
        ...
        "3. 你必须在内部完成配额、自检和修正，但最终只输出改写后的正文段落，不得输出计划、验证、统计或说明。\n"
        "4. 段落之间保留一个空行。"
    )


def _build_retry_prompt(*, plan: CnkiV20Plan, previous_output: str, issues: list[str]) -> str:
    issue_text = "；".join(item for item in issues if item) or "最终输出未通过系统校验"
    return (
        "你上一轮的知网 V20 输出未通过系统校验，必须重新执行内部改写流程并修正错误。\n"
        f"错误原因：{issue_text}\n"
...
def _repair_cnki_v20_paragraph(text: str) -> str:
    ...
    for source, target in _SOFT_PROTECTION_ANOMALIES.items():
        output = output.replace(source, target)
    output = re.sub(r"\b第一个\b", "首要", output)
    output = re.sub(r"(开展|进行|实施)(理念|观念|本质)", r"强调\2", output)
    output = re.sub(r"(把|将)\s*([A-Za-z\u4e00-\u9fff]{1,8})(开展|进行|实施)", r"\3\2", output)
    return output.strip()


def _validate_cnki_v20_output(
    *,
    plan: CnkiV20Plan,
    ...
    if original_total > 0 and rewritten_total < original_total * _MIN_OUTPUT_LENGTH_RATIO:
        raise BizError(code=4626, message="知网 V20 最终正文长度异常缩水")
    for original, rewritten, quota in zip(original_paragraphs, rewritten_paragraphs, plan.paragraphs):
        ...
        if _contains_process_markers(rewritten):
            raise BizError(code=4626, message=f"知网 V20 P{quota.index} 混入过程控制文本")
...
def _contains_forbidden_style_floor(text: str) -> bool:
    content = str(text or "")
    return any(token in content for token in _STYLE_FLOOR_FORBIDDEN)
...
def _ensure_protected_fragments(source_text: str, rewritten_text: str, *, paragraph_index: int) -> None:
    ...
    for token in protected_tokens:
        if token and token not in rewritten_text:
            raise BizError(code=4626, message=f"知网 V20 P{paragraph_index} 丢失受保护片段：{token}")
