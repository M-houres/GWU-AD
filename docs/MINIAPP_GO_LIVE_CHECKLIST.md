...
- 完成“用户隐私保护指引 / 隐私声明”配置，并明确声明：
  - 微信手机号快捷验证组件 `getPhoneNumber`
  - 用户主动从微信聊天记录中选择文件 `chooseMessageFile`
- 真机先验证一次微信隐私授权弹窗能正常通过；若报 `privacyagreement`，优先回微信公众平台补声明，不要继续误改 `app.json`
...
- 登录页协议勾选、微信隐私授权、手机号快捷登录三步都可正常完成
- 登录成功后自动进入首页
- 退出登录后重新进入要求登录
- 三类任务都能从微信聊天记录选择文件并提交成功
- 任务记录页能看到新任务
- 任务详情页状态会轮询更新
- 已完成任务可以下载并打开
- 充值套餐能正常下单
...
- 若真机仍报 `chooseMessageFile:fail api scope is not declared in the privacyagreement`，说明微信后台隐私声明尚未配置完整
